...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    // ========================== 新增：远程域名列表 ==========================
    const DOMAIN_LIST_URL = "https://你的域名列表.txt";

    axios.get(DOMAIN_LIST_URL)
      .then(r => {
        let domains = r.data
          .split(/\r?\n/)
          .map(l => l.trim())
        ...
        for (const d of domains) {
          const vlessURL =
            `vless://${UUID}@${d}:443?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${NAME}-${d}`;
          const trojanURL =
            `trojan://${UUID}@${d}:443?security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${NAME}-${d}`;

          output += vlessURL + "\n" + trojanURL + "\n";
        }

        const base64Content = Buffer.from(output).toString('base64');
